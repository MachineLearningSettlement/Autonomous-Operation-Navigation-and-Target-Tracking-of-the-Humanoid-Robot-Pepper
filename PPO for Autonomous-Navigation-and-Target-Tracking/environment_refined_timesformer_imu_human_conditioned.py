...
class PepperEnvironment:
    ...
    def build_state_graph(self, laser_scan):
        ...
        human_detected = self.detect_humans(
            self.current_frame
        ) if self.current_frame is not None else False

        ##################################################
        # Predicted Human IMU Features
        ##################################################

        if human_detected:
            imu_features = self.predict_human_imu()
        else:
            imu_features = np.zeros(6, dtype=np.float32)
        ...
        if isinstance(graph_state, dict):
            graph_state["human_imu"] = imu_features.tolist()
        else:
            graph_state = np.concatenate([
                np.asarray(graph_state, dtype=np.float32).reshape(-1),
                imu_features
            ])
