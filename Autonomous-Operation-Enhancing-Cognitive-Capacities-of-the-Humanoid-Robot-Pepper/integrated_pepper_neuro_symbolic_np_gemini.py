...
class PepperGraspingBlock:
    """
    Grasping explicitly consumes the IK output.

    The block selects a grasp approach from the detected object's geometry and
    refuses execution if IK has failed. In a physical deployment, the final
    command is sent to Pepper/NAOqi using the returned joint configuration.
    """
    ...
    def plan_and_execute(
        ...
        if not ik_result.success:
            return GraspResult(
                ...
                "none",
                [],
                "Grasp blocked because IK did not produce a valid configuration."
            )
        ...
        print(
            f"GRASPING -> target={object_label}, strategy={strategy}, "
            f"IK joints={ik_result.joint_angles}"
        )

        # The command point is intentionally the IK solution: grasping cannot
        # invent an independent arm configuration.
        return GraspResult(
            True,
            strategy,
            list(ik_result.joint_angles),
            "Grasp command generated from the validated IK configuration."
        )
